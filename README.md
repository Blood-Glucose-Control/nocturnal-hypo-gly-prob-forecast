...
The thirteen supported models do not share a single set of dependency pins (e.g. `chronos2`, `sundial`, `timesfm`, `ttm`, `moirai`, `toto` each pull in incompatible versions of `torch`, `transformers`, or `gluonts`). To handle this, the repo uses **one main virtual env plus one per-model virtual env**, all driven by `pyproject.toml` extras and a `Makefile`.
...
```bash
# 1. Main env: data loading, evaluation, analysis, naive/statistical baselines,
#    plus DeepAR / PatchTST / TFT (autogluon-based, no conflicts).
make venv-base

# 2. Per-model envs (run only the ones you need):
make venv-ttm
make venv-sundial
make venv-chronos2
make venv-timesfm
...
Each command above creates `.venvs/<model>/` from `pip install -e ".[<model>]"`.
...
```bash
# Pivot tables of RMSE / WQL / coverage / calibration plus per-horizon breakdowns.
./.venv/bin/python scripts/analysis/build_grand_summary.py

# Episode-level hypoglycemia classification table (AUROC / AUPRC for the
# any-step BG<3.9 mmol/L binary task) -- this is the main results table in
# the paper.
