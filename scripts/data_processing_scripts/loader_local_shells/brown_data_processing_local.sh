...
# Local (non-SLURM) version of brown data processing
# Run with: bash scripts/data_processing_scripts/brown_data_processing_local.sh

set -e  # Exit on error

# Setup logging - create output directory and log files with timestamp
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="cache/data/brown_2019/data_processing_job_output"
mkdir -p "$OUTPUT_DIR"
...
# Inline Python code to process the brown_2019 data
