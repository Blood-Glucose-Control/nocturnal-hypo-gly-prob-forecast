...
#
# Usage:
#   bash scripts/evaluation/sweeps/nocturnal_forecast/context_ablation/tft_ctx_ablation_eval.sh
#   GPUS="0 1" JOBS_PER_GPU=10 bash scripts/evaluation/sweeps/nocturnal_forecast/context_ablation/tft_ctx_ablation_eval.sh 2>&1 | tee logs/ctx_ablation_tft_eval.log
...
if [[ ! -f "$MANIFEST" ]]; then
    ...
    echo "       Run scripts/training/sweeps/nocturnal_forecast/context_ablation/tft_ctx_ablation_train.sh first."
    exit 1
fi
