#!/usr/bin/env bash
# Profile launcher for naive baseline forecast sweep evaluation.
#
# Canonical generic launcher:
#   scripts/evaluation/sweeps/run_sweep_eval.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
cd "$PROJECT_ROOT"

MODEL_TYPE="naive_baseline" \
TASK_FAMILY="${TASK_FAMILY:-forecasting}" \
EXPERIMENT_TYPE="${EXPERIMENT_TYPE:-nocturnal_forecast}" \
SWEEP_SPEC="configs/experiments/nocturnal_forecast/naive_baseline_forecasting_eval_sweep.yaml" \
EVAL_PYTHON="${EVAL_PYTHON:-.venvs/autogluon/bin/python}" \
JOBS_PER_GPU="${JOBS_PER_GPU:-${JOBS_PER_CPU:-4}}" \
bash scripts/evaluation/sweeps/run_sweep_eval.sh "$@"
