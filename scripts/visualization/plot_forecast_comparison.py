...
---------------------------------
- Each panel compares competing model forecasts against the same episode target.
- Columns are RMSE percentiles within each patient (easy to hard episodes).
- Rows are patients.
- Use it to see where one model consistently tracks turning points, misses
  thresholds, or drifts relative to another model.
...
Two-stage workflow to handle models that require different Python environments:
  ...
  Stage 1 — Inference (run once per model, in its model-specific env):
      ...
      python scripts/visualization/plot_forecast_comparison.py --model chronos2::Chronos2 --no-plot
      # → prints: Results cached at: results/forecast_comparisons/a3f82c1d.json

      source scripts/setup_model_env.sh ttm
      python scripts/visualization/plot_forecast_comparison.py --model ttm::TTM --no-plot
...
def run_inference(
    ...
    model, _ = create_model_and_config_fn(
        model_type,
        checkpoint,
        context_length=context_length,
        forecast_length=forecast_length,
    ...
    if to_run:
        ...
        for model_type, checkpoint, label, out_path in to_run:
            episodes = run_inference(
                ...
                args.forecast_length,
                all_episodes,
                create_model_and_config_fn=create_model_and_config,
            )
    ...
    if args.no_plot or len(cached) < 2:
        if not args.no_plot and len(cached) < 2:
            print(
                ...
                "  python scripts/visualization/plot_forecast_comparison.py --from-results "
