#!/usr/bin/env python3
"""
TiDE HPO Comparison: Manual vs Random Search vs Bayesian HPO

Generates 2 sets of 30-episode plots:
  1. 30 BEST by TiDE Manual RMSE — where the manual model shines
  2. 30 WORST by TiDE Manual RMSE — where all models struggle

Models compared:
  - TiDE Manual Scaled (512 context, 256 dims) - Baseline manual config
  - TiDE HPO Random (512 context, best config from random search HPO)
  - TiDE HPO Bayesian (512 context, best config from Ray Bayesian HPO)

Visualizes:
  - Ground truth BG (black, bold)
  - IOB trajectory (gray, secondary axis)
  - All TiDE predictions overlaid
  - Discontinuity at boundary (vertical line at t=0)
"""
...
MODELS = {
    ...
    "tide_bayesian": {
        "label": "TiDE Bayesian HPO",
        "color": "green",
        "linestyle": "-",
        "linewidth": 2.0,
        "alpha": 0.9,
        "context_length": CONTEXT_LENGTH_SCALED,
        "path": "models/tide_hpo_ray",  # Ray Bayesian HPO (Job 1406400)
        "zorder": 4,
    },
}
...
SELECTIONS = [
    {
        "name": "best30_tide_manual",
        "title_prefix": "Top 30 Episodes (Best Manual TiDE RMSE)",
        "sort_key": "rmse_tide_manual",
        "ascending": True,
        "filename": "tide_3way_best30.png",
    },
    {
        "name": "worst30_tide_manual",
        "title_prefix": "Worst 30 Episodes (Worst Manual TiDE RMSE)",
        "sort_key": "rmse_tide_manual",
        "ascending": False,
        "filename": "tide_3way_worst30.png",
    },
]
...
def plot_30_episodes(selected_episodes, model_keys, selection_config, output_dir):
    """Plot 30 episodes in a 6x5 grid with both TiDE models overlaid."""
    fig, axes = plt.subplots(6, 5, figsize=(25, 30))
    axes_flat = axes.flatten()

    # Time arrays for forecast and context
    context_steps = int(
        CONTEXT_HOURS_TO_SHOW * 60 / INTERVAL_MINS
    )  # e.g., 36 steps for 3 hours
    ...
    # Compute aggregate RMSEs and discontinuities for title
    avg_rmses = {}
    avg_disconts = {}
    for mk in model_keys:
        avg_rmses[mk] = np.mean([e[f"rmse_{mk}"] for e in selected_episodes])
        avg_disconts[mk] = np.mean([e[f"discont_{mk}"] for e in selected_episodes])
    ...
    for plot_idx, em in enumerate(selected_episodes):
        ...
        # Plot each model's predictions (only in forecast region)
        for mk in model_keys:
            cfg = MODELS[mk]
            pred = em[f"pred_{mk}"]
            ax.plot(
        ...
        # Title: show RMSE and discontinuity for manual model
        sort_rmse = em[selection_config["sort_key"]]
        manual_discont = em["discont_tide_manual"]
        ax.set_title(
            f'#{plot_idx+1} ({ep["anchor"].strftime("%Y-%m-%d")}) '
            f"IOB={em['mean_iob']:.1f}U "
            f"RMSE={sort_rmse:.2f} "
            f"Δ={manual_discont:.2f}",
            fontsize=7,
        )
    ...
    # Build subtitle with RMSEs and discontinuities
    subtitle_parts = []
    for mk in model_keys:
        subtitle_parts.append(
            f"{MODELS[mk]['label']}: RMSE={avg_rmses[mk]:.3f}, Δ={avg_disconts[mk]:.3f}"
        )
    subtitle = " | ".join(subtitle_parts)

    plt.suptitle(
        f'TiDE Validation: {selection_config["title_prefix"]}\n{subtitle}',
        fontsize=13,
        fontweight="bold",
        y=1.01,
    )
    ...
    output_path = output_dir / selection_config["filename"]
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {output_path}")

    return avg_rmses, avg_disconts


def main():
    print("=" * 70)
    print("TiDE HPO COMPARISON VISUALIZATION")
    print("Comparing Manual vs Random HPO vs Bayesian HPO")
    print("=" * 70)

    output_dir = PROJECT_ROOT / "models/tide_hpo_ray"
    ...
    for i, ep in enumerate(all_episodes):
        ...
        for mk in model_keys:
            pred = predictions[mk].loc[item_id]["mean"].values
            rmse = np.sqrt(np.mean((pred[: len(actual)] - actual) ** 2))

            # Discontinuity = |last_context_bg - first_forecast|
            discontinuity = abs(last_context_bg - pred[0])

            entry[f"pred_{mk}"] = pred
    ...
    for sel in SELECTIONS:
        ...
        avg_rmses, avg_disconts = plot_30_episodes(
            selected, model_keys, sel, output_dir
        )
        all_results[sel["name"]] = {"rmse": avg_rmses, "discont": avg_disconts}
    ...
    for sel in SELECTIONS:
        ...
        print(f"{'Model':<30s}  {'RMSE':>10s}  {'Discontinuity':>15s}")
        print("-" * 60)

        for mk in model_keys:
            rmse = all_results[sel["name"]]["rmse"][mk]
            discont = all_results[sel["name"]]["discont"][mk]
            print(f"{MODELS[mk]['label']:<30s}  {rmse:>10.4f}  {discont:>15.4f}")
