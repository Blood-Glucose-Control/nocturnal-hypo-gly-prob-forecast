...
#SBATCH --job-name="timegpt_finetune"
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=4
#SBATCH --mem-per-cpu=2GB
#SBATCH --partition=HI
#SBATCH --gres=gpu:1
#SBATCH -o results/runs/timegpt_finetune/slurm-%j.out
#SBATCH -e results/runs/timegpt_finetune/slurm-%j.err

# TimeGPT finetuning SLURM Script with YAML Configuration Support
# Usage: sbatch timegpt_finetune.sh [path/to/config.yaml]
#
# Example:
#   sbatch scripts/training/timegpt_finetune.sh configs/configs/timegpt_finetune/timegpt_baseline_config.yaml
...
# Ensure Nixtla API key is available
if [[ -z "$NIXTLA_API_KEY" ]]; then
    echo "Error: NIXTLA_API_KEY is not set. Export it in the environment before submitting."
    exit 1
fi

# Create timestamped run directory
RUN_TIMESTAMP=$(date +%Y%m%d_%H%M%S)
RUN_DIR="$HOME/nocturnal/results/runs/timegpt_finetune/run_${RUN_TIMESTAMP}_job${SLURM_JOB_ID}"
mkdir -p "$RUN_DIR"
...
python - <<'PY'
import os, sys
print(f"Python internal path check: {sys.executable}")
print(f"NIXTLA_API_KEY present: {'yes' if os.environ.get('NIXTLA_API_KEY') else 'no'}")
PY
...
python - <<PY || echo "Warning: Could not register run start"
import sys
from pathlib import Path
sys.path.append(str(Path("$HOME/nocturnal/src")))
...
registry = ModelRegistry(registry_path="results/runs/timegpt_finetune/model_registry.csv")
...
PYTHON_CMD="python $HOME/nocturnal/src/train/timegpt/timegpt.py"
...
echo "  python $HOME/nocturnal/src/train/timegpt/timegpt.py \\"
...
python - <<PY || echo "Warning: Could not update registry"
import sys, os, json
from pathlib import Path
sys.path.append(str(Path("$HOME/nocturnal/src")))
from src.utils.model_registry import ModelRegistry
...
# Move SLURM output files to run directory
echo "Moving SLURM output files to run directory..."
SLURM_OUT_FILE="results/runs/timegpt_finetune/slurm-${SLURM_JOB_ID}.out"
SLURM_ERR_FILE="results/runs/timegpt_finetune/slurm-${SLURM_JOB_ID}.err"
