#!/usr/bin/env bash
# Profile launcher for PatchTST context-ablation sweep training.
#
...
SWEEP_SPEC="configs/experiments/nocturnal_forecast/patchtst_ctx_ablation_forecasting_train_sweep.yaml" \
VENV_NAME="${VENV_NAME:-chronos2}" \
SKIP_STEPS="${SKIP_STEPS:-1 2 4 7}" \
JOBS_PER_GPU="${JOBS_PER_GPU:-2}" \
bash scripts/training/sweeps/run_sweep_train.sh \
  --artifacts-root "trained_models/artifacts/patchtst" \
