...
def check_multi_gpu_setup():
    ...
    if gpu_info["gpu_count"] > 1:
        info_print(
            f"✅ {gpu_info['gpu_count']} GPUs detected - multi-GPU training possible!"
        )
        ...
        if all_set:
            ...
            info_print("1. Use torchrun (easiest):")
            info_print(
                f"   torchrun --nproc_per_node={gpu_info['gpu_count']} scripts/examples/example_distributed_ttm.py"
            )
            ...
            try:
                result = subprocess.run(
                    ["torchrun", "--help"], capture_output=True, text=True
                )
    ...
    if gpu_info["gpu_count"] > 1 and not os.environ.get("RANK"):
        info_print(
            "   Running example_distributed_ttm.py now will use single-GPU fallback"
        )
