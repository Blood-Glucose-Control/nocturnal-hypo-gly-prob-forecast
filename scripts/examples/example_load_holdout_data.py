...
Usage:
    ...
    python scripts/examples/example_load_holdout_data.py --dataset lynch_2022 --mode train

    # Load holdout data only (for final evaluation)
    python scripts/examples/example_load_holdout_data.py --dataset lynch_2022 --mode holdout

    # Load both (for inspection)
    python scripts/examples/example_load_holdout_data.py --dataset lynch_2022 --mode both
"""
...
def load_both_example(dataset_name: str, config_dir: str):
    ...
    if "temporal_split" in split_info:
        logger.info(
            f"  Temporal holdout: {split_info['temporal_split']['holdout_percentage'] * 100}%"
        )
    if "patient_split" in split_info:
        logger.info(
            f"  Holdout patients: {split_info['patient_split']['num_holdout_patients']}"
        )
    ...
    if "patient_id" in train_data.columns:
        ...
        if overlap:
            logger.info("    ℹ Patient overlap expected for temporal/hybrid splits")
