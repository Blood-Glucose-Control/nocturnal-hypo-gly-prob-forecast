...
def example_3_load_both_splits():
    ...
    logger.info(f"  Split ratio: {len(train_data)/(len(train_data)+len(holdout_data)):.1%} train")
    ...
    if 'p_num' in train_data.columns and 'p_num' in holdout_data.columns:
        ...
        if overlap:
            ...
            logger.info(f"✓ No patient overlap - split is valid")
...
def example_7_multi_dataset_training():
    ...
    if all_holdout_data:
        ...
        for i, (dataset_name, holdout_data) in enumerate(zip(datasets, all_holdout_data)):
...
def main():
    ...
    try:
        ...
        logger.info("\nMake sure you have:")
        logger.info("  1. Generated holdout configs: python scripts/data_processing_scripts/generate_holdout_configs.py")
