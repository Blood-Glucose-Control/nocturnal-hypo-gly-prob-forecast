...
def main():
    ...
    if rank >= 0:
        info_print(f"Process info: rank {rank}, local_rank {local_rank}", rank_zero_only=False)
    else:
        info_print("No distributed environment detected (RANK not set)")
    
    # Determine if we can actually do distributed training
    can_do_distributed = (
        gpu_info["gpu_count"] > 1 and 
        rank >= 0 and world_size > 1 and master_addr is not None
    )
    ...
    if gpu_info["gpu_count"] > 1 and not can_do_distributed:
        info_print("🚨 Multi-GPU Setup Detected but Distributed Training Not Configured!")
        info_print("")
        info_print("To run distributed training, use one of these methods:")
        info_print("1. Use torchrun (recommended):")
        info_print(f"   torchrun --nproc_per_node={gpu_info['gpu_count']} --nnodes=1 scripts/examples/example_distributed_ttm.py")
        info_print("")
    ...
    if can_do_distributed:
        ...
        use_cpu = False
        info_print(f"🚀 Configured for {world_size} GPUs with DDP (rank {rank}/{world_size})")
        ...
        if gpu_info["gpu_count"] > 1:
            info_print("⚠️  Multiple GPUs detected but distributed environment not set up")
            info_print("   Run with: torchrun --nproc_per_node=2 --nnodes=1 example_distributed_ttm.py")
