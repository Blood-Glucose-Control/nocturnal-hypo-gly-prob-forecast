...
def main():
    ...
    config = TotoConfig(
        ...
        context_length=480,      # 40 hours at 5-min resolution (7.5 patches of 64)
        forecast_length=96,      # 8 hours at 5-min resolution (1.5 patches of 64)
                                 # Total: 480 + 96 = 576 = 9 patches (divisible by 64)
        batch_size=8,           # Reduced for 151M param model memory safety
        learning_rate=1e-5,     # Lower LR for numerical stability
        num_epochs=1,           # Fast iteration
        use_cpu=use_cpu,
