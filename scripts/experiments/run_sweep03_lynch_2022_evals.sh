#!/usr/bin/env bash
# Run all sweep-03 lynch_2022 evaluations sequentially (5k → 10k → 15k → 20k → 25k → 30k → 35k → 40k → 45k → 50k)
set -euo pipefail

EVAL="python scripts/experiments/nocturnal_hypo_eval.py"
COMMON_ARGS="--model chronos2 --dataset lynch_2022 --config-dir configs/data/holdout_10pct \
    --context-length 512 --forecast-length 96 --covariate-cols iob insulin_availability --cuda-device 0"

echo "=== sweep03 lynch_2022 evals ==="
...
$EVAL $COMMON_ARGS \
    ...
    --output-dir experiments/nocturnal_forecasting/512ctx_96fh/chronos2/2026-03-17_1100_recheck_03_modelpt_lynch_2022_finetuned

echo "=== All lynch_2022 evals complete ==="
