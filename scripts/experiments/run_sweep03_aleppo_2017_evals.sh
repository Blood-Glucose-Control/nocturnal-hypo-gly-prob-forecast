#!/usr/bin/env bash
# Run all sweep-03 aleppo_2017 evaluations sequentially (5k → 10k → 15k → 20k → 25k → 30k → 35k → 40k → 45k → 50k)
set -euo pipefail

EVAL="python scripts/experiments/nocturnal_hypo_eval.py"
COMMON_ARGS="--model chronos2 --dataset aleppo_2017 --config-dir configs/data/holdout_10pct \
    --context-length 512 --forecast-length 96 --covariate-cols iob insulin_availability --cuda-device 0"

echo "=== sweep03 aleppo_2017 evals ==="
...
echo "=== All aleppo_2017 evals complete ==="
