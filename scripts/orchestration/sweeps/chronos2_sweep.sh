...
#
# Usage:
#   bash scripts/orchestration/sweeps/chronos2_sweep.sh           # smoke + full sweep
#   SMOKE_ONLY=1 bash scripts/orchestration/sweeps/chronos2_sweep.sh   # smoke only, then stop
#   GPUS="0 1"   bash scripts/orchestration/sweeps/chronos2_sweep.sh
#   SMOKE_DATASET="lynch_2022" bash scripts/orchestration/sweeps/chronos2_sweep.sh
#   SKIP_SMOKETEST=1 bash scripts/orchestration/sweeps/chronos2_sweep.sh
...
if [[ "$SKIP_SMOKETEST" == "1" ]]; then
    ...
    if [[ "$SMOKE_ONLY" == "1" ]]; then
        ...
        echo "  GPUS=\"0 1\" bash scripts/orchestration/sweeps/chronos2_sweep.sh"
        exit 0
    fi
fi
