#!/usr/bin/env python3
"""
TiDE Validation Visualization: Compare TiDE Default vs Scaled

Generates 2 sets of 30-episode plots:
  1. 30 BEST by TiDE Scaled RMSE — where the scaled model shines
  2. 30 WORST by TiDE Scaled RMSE — where both models struggle

Models compared:
  - TiDE Default (144 context, 4 dims, 1 layer)
  - TiDE Scaled (512 context, 256 dims)

Visualizes:
  - Ground truth BG (black, bold)
  - IOB trajectory (gray, secondary axis)
  - Both TiDE predictions
...
MODELS = {
    "tide_default": {
        "label": "TiDE Default (144c, 4d)",
        "color": "blue",
        "linestyle": "-",
        "linewidth": 1.8,
        "alpha": 0.8,
        "context_length": CONTEXT_LENGTH_DEFAULT,
        "path": "models/tide_validation/default",
        "zorder": 2,
    },
    "tide_scaled": {
        "label": "TiDE Scaled (512c, 256d)",
        "color": "darkorange",
        "linestyle": "-",
        "linewidth": 2.0,
        "alpha": 0.9,
...
SELECTIONS = [
    {
        "name": "best30_tide_scaled",
        "title_prefix": "Top 30 Episodes (Best TiDE Scaled RMSE)",
        "sort_key": "rmse_tide_scaled",
        "ascending": True,
        "filename": "tide_best30.png",
    },
    {
        "name": "worst30_tide_scaled",
        "title_prefix": "Worst 30 Episodes (Worst TiDE Scaled RMSE)",
        "sort_key": "rmse_tide_scaled",
        "ascending": False,
        "filename": "tide_worst30.png",
    },
]
...
def plot_30_episodes(selected_episodes, model_keys, selection_config, output_dir):
    ...
    for plot_idx, em in enumerate(selected_episodes):
        ...
        # Title: show RMSE and discontinuity for scaled model
        sort_rmse = em[selection_config["sort_key"]]
        scaled_discont = em["discont_tide_scaled"]
        ax.set_title(
            f'#{plot_idx+1} ({ep["anchor"].strftime("%Y-%m-%d")}) '
            f"IOB={em['mean_iob']:.1f}U "
            f"RMSE={sort_rmse:.2f} "
            f"Δ={scaled_discont:.2f}",
            fontsize=7,
        )
...
def main():
    print("=" * 70)
    print("TiDE VALIDATION VISUALIZATION")
    print("Comparing TiDE Default vs Scaled with discontinuity analysis")
    print("=" * 70)

    output_dir = PROJECT_ROOT / "models/tide_validation"
