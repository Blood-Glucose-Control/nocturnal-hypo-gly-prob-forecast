#!/usr/bin/env python3
"""End-to-end holdout workflow for time series forecasting models.

Runs the seven-step pipeline against any registered model:
  ...
  7. Resume training on the loaded checkpoint

Individual steps can be skipped via --skip-steps. Step 4 is auto-skipped for
models that train from scratch (e.g., timegrad, tide), since they have no
pretrained weights for zero-shot inference.

Examples:
    python examples/end_to_end_workflow.py --model-type ttm \\
        --datasets lynch_2022 brown_2019

    python examples/end_to_end_workflow.py --model-type tide \\
        --datasets aleppo_2017 brown_2019 lynch_2022 tamborlane_2008

    python examples/end_to_end_workflow.py --model-type ttm \\
        --datasets lynch_2022 --skip-steps 4 7

    python examples/end_to_end_workflow.py --model-type timegrad \\
        --datasets lynch_2022
"""
...
def load_model_config_from_yaml(config_path: str) -> Dict[str, Any]:
    ...
    Reads a YAML model config file (e.g., configs/models/ttm/00_fine_tune_cgm_only.yaml)
...
Step 4 is auto-skipped for from-scratch models (e.g., timegrad, tide).

Each evaluation phase (4, 5, 6, 7) writes its predictions and plots into
its own subdirectory for comparison.
        """,
    )
    parser.add_argument(
        "--model-type",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to YAML model configuration file (e.g., configs/models/ttm/00_fine_tune_cgm_only.yaml)",
    )
    ...
    if args.output_dir is None:
        args.output_dir = f"./trained_models/artifacts/{args.model_type}/{_run_subdir}"
    else:
        args.output_dir = str(Path(args.output_dir) / _run_subdir)

    logger.info("=" * 80)
    logger.info("END-TO-END HOLDOUT WORKFLOW")
    logger.info("=" * 80)
