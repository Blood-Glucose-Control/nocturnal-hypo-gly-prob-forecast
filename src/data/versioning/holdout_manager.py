...
class HoldoutManager:
    ...
    def _apply_temporal_split(
        ...
        holdout_data = pd.concat(holdout_dfs, ignore_index=True) if holdout_dfs else pd.DataFrame()
    ...
    def _apply_patient_split(
        ...
        if config.has_predefined_patients():
            ...
            if config.holdout_percentage is None:
                raise ValueError("Either holdout_patients or holdout_percentage must be specified")
        ...
        # Combine data
        train_dfs = [patient_data[p] for p in train_patients]
        holdout_dfs = [patient_data[p] for p in holdout_patients]
        ...
        logger.info(f"Train patients: {train_patients}")
    ...
    def _apply_hybrid_split(
        ...
        holdout_data = pd.concat(
            [temporal_holdout_data, holdout_patients_data],
            ignore_index=True
        )
        
        logger.info(
            f"Hybrid split: {len(train_data)} train samples from {len(self._train_patients)} patients, "
    ...
    def validate_split(
        ...
        if self.config.holdout_type == HoldoutType.TEMPORAL:
            ...
            if patient_col in train_data.columns and patient_col in holdout_data.columns:
                ...
                if overlap:
                    logger.error(f"Patient overlap detected in patient-based split: {overlap}")
                    results["no_overlap"] = False
            else:
                logger.warning(f"Cannot validate patient overlap: '{patient_col}' column not found")
                results["no_overlap"] = False
            ...
            if self._holdout_patients and patient_col in train_data.columns:
                ...
                train_patients_str = set(str(p) for p in train_data[patient_col].unique())
                ...
                logger.warning("Cannot validate hybrid split: patient column or list not available")
                results["no_overlap"] = False
...
def generate_patient_holdout_list(
    ...
    holdout_patients = rng.choice(
        all_patients, size=n_holdout, replace=False
    ).tolist()
