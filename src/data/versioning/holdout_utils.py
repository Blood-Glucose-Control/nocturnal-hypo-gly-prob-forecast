    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            if 'p_num' in data.columns:
                ...
                logger.info(f"Data is DataFrame with 'p_num' column, {len(patient_ids)} unique patients")
            elif 'patient_id' in data.columns:
                patient_ids = data['patient_id'].unique().tolist()
                logger.info(f"Data is DataFrame with 'patient_id' column, {len(patient_ids)} unique patients")
            else:
                logger.warning(f"No patient column found in {dataset_name}, treating as single patient")
                patient_ids = ['patient_1']
        else:
            logger.warning(f"Unknown data format for {dataset_name}: {type(data)}, treating as single patient")
            patient_ids = ['patient_1']
    ...
    try:
        ...
        if 'p_num' in train_data.columns and 'p_num' in holdout_data.columns:
            ...
            if config.holdout_type.value == "patient":
                ...
                if overlap:
                    results["errors"].append(f"Patient overlap detected: {len(overlap)} patients")
                    if verbose:
                        logger.error(f"✗ Patient overlap detected: {len(overlap)} patients")
                else:
                    results["no_data_leakage"] = True
                    if verbose:
                        logger.info(f"✓ No patient overlap (patient-based split)")
                        
            elif config.holdout_type.value == "hybrid":
                # Hybrid splits: designated holdout patients should NOT appear in training
                if config.patient_config and hasattr(config.patient_config, 'holdout_patients'):
                    designated_holdout = set(str(p) for p in config.patient_config.holdout_patients)
                    
                    # Check if any designated holdout patients appear in training set
                    leak = designated_holdout & set(train_patients)
                    if leak:
                        results["errors"].append(f"Designated holdout patients in training: {leak}")
                        if verbose:
                            logger.error(f"✗ Designated holdout patients leaked: {leak}")
                    else:
                        results["no_data_leakage"] = True
                        if verbose:
                            logger.info(f"✓ Hybrid split valid: {len(overlap)} patients overlap (temporal)")
                else:
                    results["no_data_leakage"] = True
                    if verbose:
                        logger.info(f"✓ Hybrid split: {len(overlap)} patients overlap (expected)")
            else:
                # Temporal splits: overlap is EXPECTED
                results["no_data_leakage"] = True
                if verbose:
                    logger.info(f"✓ Temporal split: {len(overlap)} patients overlap (expected)")
        else:
            # Temporal-only splits
            results["no_data_leakage"] = True
            if verbose:
                logger.info(f"✓ Temporal-only split (no patient column)")
        
        # Validate minimum samples
        if results["train_size"] < 100:
            results["errors"].append(f"Training set very small: {results['train_size']} samples")
            if verbose:
                logger.warning(f"⚠ Training set very small")
        
        if results["holdout_size"] < 20:
            results["errors"].append(f"Holdout set very small: {results['holdout_size']} samples")
            if verbose:
                logger.warning(f"⚠ Holdout set very small")
        
        # Check temporal ordering for temporal splits
        if config.temporal_config and 'time' in train_data.columns and 'time' in holdout_data.columns:
            try:
                ...
                if 'p_num' in train_data.columns:
                    ...
                    for patient in train_patients:
                        ...
                        if len(patient_train) > 0:
                            ...
                            patient_holdout = holdout_data[holdout_data['p_num'] == patient]
                    ...
                    if temporal_issues:
                        ...
                        if verbose:
                            logger.error(f"✗ Temporal ordering issue for patients: {temporal_issues}")
                    else:
                        if verbose:
                            logger.info(f"✓ Temporal ordering correct for all patients")
                else:
                    max_train_time = train_data['time'].max()
                    min_holdout_time = holdout_data['time'].min()
                    if max_train_time >= min_holdout_time:
                        results["errors"].append("Temporal ordering issue: train extends into holdout period")
                        if verbose:
                            logger.error(f"✗ Temporal ordering issue")
                    else:
                        if verbose:
                            logger.info(f"✓ Temporal ordering correct")
            except Exception as e:
                if verbose:
