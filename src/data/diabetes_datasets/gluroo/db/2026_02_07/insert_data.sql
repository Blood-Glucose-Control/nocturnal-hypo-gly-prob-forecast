/*
=============================================================================
Import All Data into TimescaleDB (Groups, Messages, Readings)
=============================================================================

USAGE: Run this script to import all CSV data into the database
NOTE: This script reads .csv.gz files directly using zcat

Using Apptainer (inside the container):
    psql -h 127.0.0.1 -U postgres -d gluroo_datasets -f /workspace/src/data/diabetes_datasets/gluroo/db/2026_02_07/insert_data.sql

Using Docker:
  docker exec timescaledb psql -U postgres -d gluroo_datasets -f /workspace/src/data/diabetes_datasets/gluroo/db/2026_02_07/insert_data.sql

IMPORTANT: After running this, run add_patient_id.sql to assign p_num values
...
-- Import groups data
-- Using FROM PROGRAM 'zcat' to read .csv.gz files directly without decompressing to disk
-- Note: zcat also works with uncompressed .csv files if you ever switch back
...
\COPY readings FROM PROGRAM 'zcat /data/shared/cache/data/gluroo_2026/raw/readings.csv.gz' WITH (FORMAT csv, HEADER true, DELIMITER ',');
