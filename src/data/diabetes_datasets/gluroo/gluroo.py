...
TODO: Double check patient_id refracotring logic.
...
class GlurooDataLoader(DatasetBase):
    ...
    - patient_id (the internal id) the patient now a string identifier (e.g., gluroo_1) ordered by gid (generated by the script add_patient_id.sql).
    - patient_id is the patient_id
    ...
    def load_raw(
        ...
        if patient_ids is None:
            ...
            patient_ids_patient_ids = self._get_all_patient_ids()
            logger.info(
                f"Loading raw data for all {len(patient_ids_patient_ids)} groups from TimescaleDB..."
            )
            # Extract just the gids for iteration
            patient_ids = [gid for gid, _patient_id in patient_ids_patient_ids]
        else:
            logger.info(
    ...
    def get_hf_streaming_dataset(
        ...
        Args:
            ...
            patient_ids: Optional iterable of patient_id identifiers to filter.
        ...
        if validate_non_empty:
            try:
                ...
                raise ValueError(
                    ...
                    f"Filters -> columns={columns}, patient_id filter={patient_ids}."
                ) from e
    ...
    def _load_all_into_processed_data(self) -> None:
        ...
        Reads all *.parquet under the cache parquet path and groups by patient_id.
        """
        processed_path = self.cache_manager.get_absolute_path_by_type(
            self.dataset_name, "processed"
        )
        ...
        for path in parquet_files:
            ...
            if ColumnNames.P_NUM.value not in df.columns:
                logger.warning(f"File {path} missing patient_id column; skipping.")
                continue
            for patient_id, group in df.groupby(ColumnNames.P_NUM.value, sort=False):
                if patient_id not in self.processed_data:
                    self.processed_data[str(patient_id)] = group.copy()
                else:
                    self.processed_data[str(patient_id)] = pd.concat(
                        [self.processed_data[str(patient_id)], group], ignore_index=True
                    )
    ...
    def _get_all_patient_ids(self) -> list[tuple[str, str]]:
        ...
        Returns:
            list[tuple[str, str]]: List of patient tuples (gid, patient_id) sorted by gid (each gid is mapped to one string patient_id)
        """
        engine = create_engine(self.db_connection_string)
        with engine.connect() as conn:
            ...
            result = conn.execute(
                text("SELECT DISTINCT gid, patient_id FROM groups ORDER BY gid")
            )
            gids_patient_ids = [(row[0], str(row[1])) for row in result.fetchall()]
        return gids_patient_ids

    def _get_patient_id_mapping(self) -> dict[str, str]:
        """
        Get mapping from gid to string patient_id from the database.

        Returns:
            dict[str, str]: Dictionary mapping gid to patient_id (string, ordered) (each gid is mapped to one patient_id)
        """
        engine = create_engine(self.db_connection_string)
        with engine.connect() as conn:
            # Get gid and patient_id from groups table, ordered by patient_id for consistency
            result = conn.execute(
                text("SELECT gid, patient_id FROM groups ORDER BY patient_id")
            )
    ...
    def _load_patient_data_from_db(self, engine, gid: str) -> pd.DataFrame | None:
        ...
        # Join with groups table to get string patient_id (This is generated by the script add_patient_id.sql)
        query = text("""
            SELECT
                r.date,
                r.bgl,
        ...
        try:
            with engine.connect() as conn:
                ...
                df[ColumnNames.P_NUM.value] = df["patient_id"]
                return df
    ...
    def _process_and_cache_data(self, batch_size: int = 10) -> dict[str, pd.DataFrame]:
        ...
        """
        # Get all patient IDs with patient_ids first (sorted by gid)
        all_patient_ids_patient_ids = self._get_all_patient_ids()
        total_patients = len(all_patient_ids_patient_ids)
        ...
        for batch_start in range(0, total_patients, batch_size):
            ...
            batch_patient_ids_patient_ids = all_patient_ids_patient_ids[
                batch_start:batch_end
            ]
            # Extract just the gids for load_raw
            batch_patient_ids = [
                gid for gid, _patient_id in batch_patient_ids_patient_ids
            ]
            ...
            #### From this point on, we only use patient_id to identify patients. gid is not used anymore. #####
            # Convert from {gid: df} to {patient_id: df} for processing
            batch_raw_data = {}
            ...
            for gid, df in batch_raw_data_gid.items():
                if "patient_id" not in df.columns or df.empty:
                    logger.warning(
                        f"Patient {gid} missing patient_id column or empty DataFrame. Skipping..."
                    )
                    continue
                patient_id = str(df["patient_id"].iloc[0])
                batch_raw_data[patient_id] = df
                gid_to_pnum_map[patient_id] = gid

            # Process this batch with parallel processing (now uses patient_id as keys)
            ...
            self._save_batch_incremental(
                ...
                batch_raw_data,  # Pass the patient_id-keyed dict for reference
                parquet_path,
                batch_num,
            )
    ...
    def _process_raw_data_batch(
        ...
        Args:
            raw_data: Dictionary mapping patient_id (string) to raw DataFrames
            processed_path: Path to processed data directory (not used here, but kept for compatibility)
            gid_to_pnum_map: Dictionary mapping patient_id to gid for error messages

        Returns:
            dict[str, pd.DataFrame]: Dictionary mapping patient_id to processed DataFrames
        """
        processed_data = {}

        # Prepare patient data tuples for parallel processing
        # Sort by patient_id to ensure deterministic ordering (probably not needed because data should already be sorted by patient_id)
        patient_tuples = sorted(
            [(patient_id, df, processed_path) for patient_id, df in raw_data.items()],
            key=lambda x: self._patient_id_to_int(
                x[0]
            ),  # Sort by numeric portion of patient_id
        )

        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all tasks and track them by patient_id
            future_to_pnum = {
                executor.submit(
                    process_single_patient,
                    patient_tuple,
                    # Data: (patient_id, df, processed_path)
            ...
            for index, future in enumerate(as_completed(future_to_pnum), 1):
                patient_id = future_to_pnum[future]
                gid = gid_to_pnum_map.get(patient_id, "unknown")
                progress = f"({index}/{len(raw_data)})"
                try:
                    result_pnum, df = future.result()
                    if df.empty:
                        logger.warning(
                            f"Processed data is empty for patient patient_id={result_pnum} (gid={gid})"
                        )
                        continue
                    results[result_pnum] = df
                    logger.debug(
                        f"Successfully processed patient patient_id={result_pnum} (gid={gid}) {progress}"
                    )
                except Exception as exc:
                    logger.error(
                        f"Patient patient_id={patient_id} (gid={gid}) generated an exception: {exc} {progress}"
                    )

            # Reconstruct processed_data in deterministic order (sorted by patient_id)
            for patient_id, _df, _processed_path in patient_tuples:
                if patient_id in results:
                    processed_data[patient_id] = results[patient_id]

        return processed_data

    def _process_one_patient(
        self, df_raw: pd.DataFrame, patient_id: str, gid: str = "unknown"
    ) -> pd.DataFrame | None:
        """
        Process raw data for a single patient.

        Args:
            df_raw: Raw DataFrame for the patient (must contain patient_id column from database)
            patient_id: Patient number (string identifier)
        ...
        """
        try:
            # Verify patient_id matches DataFrame
            if "patient_id" not in df_raw.columns:
                raise ValueError(
                    f"DataFrame missing patient_id column for patient patient_id={patient_id} (gid={gid})"
                )
            df_patient_id = str(df_raw["patient_id"].iloc[0])
            if df_patient_id != patient_id:
                logger.warning(
                    f"patient_id mismatch: expected {patient_id}, got {df_patient_id} for patient gid={gid}"
                )

            # Translate to standardized format
            df = data_translation(df_raw)

            # Apply preprocessing pipeline using patient_id
            df = preprocessing_pipeline(str(patient_id), df, use_aggregation=True)

            # df.to_csv(f"processed_{patient_id}.csv")

            return df
        except Exception as e:
            logger.error(
                f"Error processing patient patient_id={patient_id} (gid={gid}): {e}"
            )
            return None

    def _patient_id_to_int(self, patient_id: int | str) -> int:
        """
        Extract integer component from patient_id string (e.g., gluroo_123 -> 123).
    ...
    def _get_partition_number(
        ...
        Accepts either numeric index or patient_id string (e.g., gluroo_123).
        """
        numeric_index = self._patient_id_to_int(patient_index)
    ...
    def _save_batch_incremental(
        ...
        It determines partition numbers based on string patient_id from the database.

        Args:
            batch_processed_data: Dictionary mapping patient_id (str) to processed DataFrames for this batch
            batch_raw_data: Dictionary mapping patient_id (str) to raw DataFrames (used to extract patient_id for patient_id column)
        ...
        """
        # Group batch patients by partition using numeric component of patient_id
        partition_groups: dict[int, list[tuple[str, pd.DataFrame]]] = {}
        for patient_id in batch_processed_data.keys():
            if patient_id not in batch_processed_data:
                continue  # Skip if processing failed

            partition_num = self._get_partition_number(
                patient_id, patients_per_partition
            )

            if partition_num not in partition_groups:
                partition_groups[partition_num] = []
            partition_groups[partition_num].append(
                (patient_id, batch_processed_data[patient_id])
            )
        ...
        for partition_num in sorted(partition_groups.keys()):
            ...
            partition_data = []
            for patient_id, df in partition_groups[partition_num]:
                df_with_id = df.copy()
                if "date" in df_with_id.columns:
                    df_with_id = df_with_id.drop(columns=["date"])
                df_with_id[ColumnNames.P_NUM.value] = patient_id
    ...
    def _save_as_partitioned_parquet(
        ...
        for partition_num in sorted(partition_groups.keys()):
            ...
            for patient_id, df in patients:
                # Add patient_id column; drop redundant date if present
                df_with_id = df.copy()
                if "date" in df_with_id.columns:
                    df_with_id = df_with_id.drop(columns=["date"])
                df_with_id[ColumnNames.P_NUM.value] = patient_id
...
def process_single_patient(patient_tuple: tuple[str, pd.DataFrame, Path]) -> tuple:
    ...
    Args:
        patient_tuple: Tuple containing (patient_id, df_raw, processed_path)

    Returns:
        tuple: (patient_id, processed_df) - returns patient_id (str) for mapping
    """
    patient_id, df_raw, _processed_path = patient_tuple

    if df_raw.empty:
        # This shouldn't really happen. But just in case.
        logger.warning(
            f"Raw data is empty for patient patient_id={patient_id}. Skipping..."
        )
        return (patient_id, None)

    # Extract patient_id from DataFrame to verify it matches
    if "patient_id" not in df_raw.columns:
        logger.error(
            f"DataFrame missing patient_id column for patient patient_id={patient_id}. Skipping..."
        )
        return (patient_id, None)
    ...
    if df_patient_id != patient_id:
        logger.warning(
            f"patient_id mismatch: expected {patient_id}, got {df_patient_id}"
        )

    logger.info(f"Processing patient patient_id={patient_id}")

    # Process the patient
    try:
        df = data_translation(df_raw)
        df = preprocessing_pipeline(str(patient_id), df, use_aggregation=True)
        # Debugging only: use patient_id for filename (cleaner than gid)
        # df.to_csv(f"processed_{patient_id}.csv", index=True)  # Debugging only

        if df.empty:
            raise ValueError(
                f"Processed data is empty for patient patient_id={patient_id}"
            )

        logger.debug(f"Done processing patient patient_id={patient_id}")

        return (patient_id, df)
    except Exception as e:
        logger.error(f"Error processing patient patient_id={patient_id}: {e}")
        raise
