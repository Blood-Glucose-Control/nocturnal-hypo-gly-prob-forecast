...
def clean_tamborlane_2008_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'device_date' in data.columns and 'device_time' in data.columns:
        # Combine date and time, handling potential format issues
        try:
        ...
        logger.info(f"Created datetime column from device_date and device_time")
        ...
        logger.info(f"Created datetime column from display_time")
...
def extract_cgm_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'bg_mM' in df.columns:
        ...
        df['glucose_cv'] = df['glucose_1h_std'] / df['glucose_1h_mean']  # Coefficient of variation
...
def validate_tamborlane_data(df: pd.DataFrame) -> Dict[str, any]:
    ...
    if 'p_num' in df.columns and ('bg_mM' in df.columns or 'bg_mg_dl' in df.columns):
        ...
        metrics['mean_readings_per_patient'] = patient_stats[(glucose_col, 'count')].mean()
        metrics['std_readings_per_patient'] = patient_stats[(glucose_col, 'count')].std()
        metrics['min_readings_per_patient'] = patient_stats[(glucose_col, 'count')].min()
        metrics['max_readings_per_patient'] = patient_stats[(glucose_col, 'count')].max()
...
def prepare_for_modeling(df: pd.DataFrame, 
    ...
    feature_cols = [col for col in df.columns if col.startswith(('bg_lag_', 'roc_lag_', 'glucose_'))]
