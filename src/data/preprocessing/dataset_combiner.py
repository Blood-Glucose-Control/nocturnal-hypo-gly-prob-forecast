...
def combine_datasets_for_training(
    ...
    for dataset_name in dataset_names:
        ...
        try:
            ...
            logger.info(
                f"    ✓ Loaded: {len(data):,} samples, {len(data.columns)} columns"
            )
...
def print_dataset_column_table(
    column_info: Dict[str, List[str]], final_columns: List[str]
):
    """
    ...
    logger.info(
        f"  Common across all: {len(set(final_columns) - {'source_dataset'})} columns"
    )
    logger.info("=" * 80 + "\n")
