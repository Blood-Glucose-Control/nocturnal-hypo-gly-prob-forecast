...
class BaseTransformer(ABC):
    ...
    def fit_transform(self, X: Union[pd.Series, pd.DataFrame], y=None) -> Union[pd.Series, pd.DataFrame]:
        """
...
class PatientGroupTransformer(BaseTransformer):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not transformed_data:
            return X.copy()  # Return a copy of the original if no transformations were done
