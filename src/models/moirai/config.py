...
class MoiraiConfig(ModelConfig):
    ...
    Attributes:
        ...
        finetune_pattern: Which layers to fine-tune. Options: ``"full"``
            (all layers), ``"freeze_ffn"`` (freeze feed-forward layers),
            ``"head_only"`` (only train the output projection head).

        --- Data helpers ---
        interval_mins: CGM sampling interval in minutes (typically 5).
    ...
    def __init__(self, **kwargs):
        ...
        self.finetune_pattern: str = kwargs.get("finetune_pattern", "full")
        ...
        # Sub-configs (handle both dataclass instances and dicts from JSON)
        tc = kwargs.get("training_config", MoiraiTrainingConfig())
        self.training_config: MoiraiTrainingConfig = (
            MoiraiTrainingConfig(**tc) if isinstance(tc, dict) else tc
        )
        dc = kwargs.get("data_config", MoiraiDataConfig())
        self.data_config: MoiraiDataConfig = (
