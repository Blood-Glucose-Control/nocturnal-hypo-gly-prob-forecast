...
def load_lora_state_dict(
    model: nn.Module, lora_state_dict: Dict[str, torch.Tensor]
) -> None:
    """
...
def save_lora_adapters(
    model: nn.Module, save_directory: str, config: Optional[Dict[str, Any]] = None
) -> None:
    """
...
def load_lora_adapters(model: nn.Module, adapter_directory: str) -> Dict[str, Any]:
    """
...
class LoRATrainer:
    ...
    def __init__(
        ...
        self.model = apply_lora_to_model(
            self.model, target_modules=self.target_modules, rank=rank, alpha=alpha
        )
...
def get_time_series_lora_config(rank: int = 16, alpha: int = 32) -> Dict[str, Any]:
    ...
    return {
        ...
        "target_modules": [
            ...
            "fc",
            "linear",
            "projection",  # Feed-forward layers
            "mixer",
            "temporal_mixer",  # Time series specific layers
        ],
        "dropout": 0.1,
        "bias": "none",
    }
