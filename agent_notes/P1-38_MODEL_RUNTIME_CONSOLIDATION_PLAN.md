...
functions). Values in model columns are implementation LOC for that
method/function (`x` = not present). Use this as a consolidation map when
deciding what to promote to base classes or shared utility modules.
Current snapshot includes **146** distinct methods/functions across the seven
family modules.

<details>
<summary>Expand full method/function presence grid</summary>

| Method / function | Purpose | Promote and Combine candidate | TTM | TimesFM | Moirai | Moment | Chronos2 | Toto | Tide |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| __init__ | Initialize forecaster runtime state and configuration. | Promote to shared base lifecycle contract. | 25 | 4 | 26 | 26 | 10 | x | 8 |
| training_backend | Report which backend is used for model training. | Promote to shared base lifecycle contract. | 7 | 2 | 3 | 12 | 2 | 2 | 2 |
| supports_zero_shot | Report whether zero-shot inference is supported. | Promote to shared base lifecycle contract. | 2 | 2 | 3 | 2 | 2 | 2 | 2 |
| supports_probabilistic_forecast | Report whether probabilistic forecasts are supported. | Promote to shared base lifecycle contract. | x | 2 | 3 | 2 | 2 | 2 | 2 |
| _initialize_model | Initialize or hydrate model weights/components for runtime use. | Promote to shared base lifecycle contract. | 58 | 35 | 48 | 24 | 4 | 19 | 4 |
| _prepare_training_data | Prepare normalized training/validation/test inputs for the backend. | Promote to shared base lifecycle contract. | 51 | 74 | 28 | 39 | 54 | 49 | 40 |
| _train_model | Execute model training/fine-tuning for the current backend. | Promote to shared base lifecycle contract. | 56 | 48 | 168 | 191 | 93 | 35 | 44 |
| _predict | Run single-input inference and return forecast outputs. | Promote to shared base lifecycle contract. | 67 | 104 | 14 | 31 | 32 | 58 | 30 |
| _predict_batch | Run multi-episode inference and return per-episode outputs. | Promote to shared base lifecycle contract. | 46 | x | x | 64 | 43 | 29 | 31 |
| _save_checkpoint | Persist model state and required runtime artifacts. | Promote to shared base lifecycle contract. | 13 | 12 | 20 | 4 | 21 | 17 | 13 |
| _load_checkpoint | Restore model state and required runtime artifacts. | Promote to shared base lifecycle contract. | 66 | 10 | 48 | 39 | 38 | 34 | 15 |
| _ag_item_id | Map an episode ID to the model item_id used for extraction. | x | x | x | x | x | 10 | x | x |
| _autogluon_extract | Run fine-tuned model inference and extract specified columns. | Combine candidate with: _extract_episode_predictions, _episode_predictions_frame, _collect_batch_predictions. | x | x | x | x | 43 | x | x |
| _build_autogluon_frequency | Convert interval minutes to model frequency string. | x | x | x | x | x | x | x | 3 |
| _build_autogluon_predict_kwargs | Helper for forecast generation/extraction. | x | x | x | x | x | 9 | x | x |
| _build_batch_inputs | Build a padded MaskedTimeseries batch for episode-level inference. | x | x | x | x | x | x | 53 | x |
| _build_context_matrix | Build finite context matrix [time, channels] from selected columns. | Combine candidate with: _get_input_columns, _get_covariate_columns, _normalize_context_array. | x | x | x | 9 | x | x | x |
| _build_context_target_dataset | Helper for context/covariate shaping. | Combine candidate with: _build_context_target_loader, _get_context_target_pairs, _split_context_target_pairs. | x | x | x | 24 | x | x | x |
| _build_context_target_loader | Helper for context/covariate shaping. | Combine candidate with: _build_context_target_dataset, _get_context_target_pairs, _split_context_target_pairs. | x | x | x | 19 | x | x | x |
| _build_data_loader | Helper for data normalization or dataset assembly. | Potential pairwise combine candidate. | 7 | 6 | x | x | x | x | x |
| _build_finetuning_module | Construct the model Lightning finetuning module from current backbone. | x | x | x | x | x | x | 18 | x |
| _build_fit_kwargs | Build fit kwargs for TimeSeriesPredictor.fit(). | x | x | x | x | x | x | x | 11 |
| _build_forecast_inputs | Helper for forecast generation/extraction. | x | x | x | x | 47 | x | x | x |
| _build_hf_trainer | Helper for training setup/execution. | x | x | 18 | x | x | x | x | x |
| _build_known_covariates | Build known_covariates DataFrame for all episodes in data. | Combine candidate with: _build_autogluon_predict_kwargs, _prepare_autogluon_data, _collect_batch_predictions. | x | x | x | x | 54 | x | x |
| _build_optional_data_loader | Helper for data normalization or dataset assembly. | x | 9 | x | x | x | x | x | x |
| _build_prediction_context | Build model context frame with item/timestamp index. | Combine candidate with: build_autogluon_context_frame (shared), _predict_with_context, _collect_batch_predictions. | x | x | x | x | x | x | 16 |
| _build_predictor_kwargs | Build TimeSeriesPredictor constructor kwargs for model. | x | x | x | x | x | x | x | 12 |
| _build_shadow_predictor_snapshot | Helper for forecast generation/extraction. | x | x | x | x | x | 70 | x | x |
| _build_temporal_eval_dataset | Build temporal eval windows for mid-training callback. | Combine candidate with: _extract_ground_truth, _build_window_dataset, evaluate. | x | 58 | x | x | x | x | x |
| _build_train_val_arrays | Split patient segments into training and validation arrays. | Combine candidate with: _split_train_val_patients, _extract_patient_dataframes, _segment_patient_data. | x | 36 | x | x | x | x | x |
| _build_trainer | Helper for training setup/execution. | x | 15 | x | x | x | x | x | x |
| _build_trainer_kwargs | Build trainer kwargs using either epoch- or step-based limits. | Combine candidate with: _create_training_arguments, _build_hf_trainer, _build_training_callbacks. | x | x | x | x | x | 26 | x |
| _build_trainer_model | Helper for training setup/execution. | x | x | 11 | x | x | x | x | x |
| _build_training_callbacks | Helper for training setup/execution. | x | x | 29 | x | x | x | x | x |
| _build_training_datasets | Build train/val/test datasets from normalized training input. | x | 18 | x | x | x | x | x | x |
| _build_variates | Extract BG target + covariate tensors from a single episode. | x | x | x | x | x | x | 11 | x |
| _build_window_dataset | Helper for data normalization or dataset assembly. | x | x | 13 | x | x | x | x | x |
| _build_zero_shot_batch_context_tensor | Build a padded (N, 1, L) context tensor for zero-shot batch inference. | x | x | x | x | x | 28 | x | x |
| _build_zero_shot_pipeline | Create the zero-shot inference pipeline with shared settings. | Combine candidate with: _build_zero_shot_pipeline_for_data, _prepare_zero_shot_context, _zero_shot_forecast. | 20 | x | x | x | x | x | x |
| _build_zero_shot_pipeline_for_data | Build a zero-shot pipeline and return the resolved target column. | Combine candidate with: _build_zero_shot_pipeline, _prepare_zero_shot_context, _zero_shot_forecast. | 24 | x | x | x | x | x | x |
| _capture_training_history | Capture in-memory trainer history for metadata/reporting. | x | 16 | x | x | x | x | x | x |
| _checkpoint_config_payload | Helper for checkpoint path/state handling. | Combine candidate with: _checkpoint_paths, _write_checkpoint_config, _load_saved_checkpoint_config. | x | 8 | x | x | x | x | x |
| _checkpoint_paths | Helper for checkpoint path/state handling. | Combine candidate with: _checkpoint_config_payload, _load_saved_checkpoint_config, _write_checkpoint_config. | x | 5 | x | x | x | x | x |
| _collate_fn | Custom collator for HF Trainer. | x | x | 14 | x | x | x | x | x |
| _collect_batch_predictions | Collect per-episode outputs from model predictions. | x | x | x | x | x | x | x | 20 |
| _collect_zero_shot_batch_results | Collect per-episode numpy outputs from model zero-shot tensors. | x | x | x | x | x | 15 | x | x |
| _compute_trainer_metrics | Compute evaluation metrics for Trainer. | x | 118 | x | x | x | x | x | x |
| _compute_validation_loss | Model-specific runtime helper. | x | x | x | x | 26 | x | x | x |
| _configure_training_environment | Set runtime environment knobs for stable Trainer execution. | x | 18 | x | x | x | x | x | x |
| _convert_tensors_to_patched_dataset | Helper for data normalization or dataset assembly. | x | x | x | 72 | x | x | x | x |
| _create_column_specifiers | Create column specifiers for TimeSeriesPreprocessor. | x | 42 | x | x | x | x | x | x |
| _create_training_arguments | Create TrainingArguments for model training. | Combine candidate with: _build_trainer_kwargs, _build_hf_trainer, _build_training_callbacks. | 46 | 22 | x | x | x | x | x |
| _create_training_artifacts | Create checkpoint and logger artifacts for fine-tuning. | Combine candidate with: _restore_trained_backbone, _build_finetuning_module, _run_forecast. | x | x | x | x | x | 20 | x |
| _dataframe_to_gluonts | Convert a single-episode DataFrame to a one-entry GluonTS dataset. | x | x | x | 39 | x | x | x | x |
| _dataframe_to_hf_dataset | Convert a flat DataFrame to HuggingFace Dataset format for model. | x | x | x | x | x | x | 66 | x |
| _empty_training_tensors | Helper for training setup/execution. | x | x | x | 20 | x | x | x | x |
| _ensure_datetime_index | Ensure each patient frame has DatetimeIndex when available. | x | x | 15 | x | x | x | x | x |
| _ensure_training_preprocessor | Create or validate the training preprocessor. | x | 19 | x | x | x | x | x | x |
| _ensure_zs_pipeline | Lazily initialise the zero-shot Chronos2Pipeline. | x | x | x | x | x | 12 | x | x |
| _episode_ids_from | Return the array of episode IDs present in *data*. | x | x | x | x | x | 5 | x | x |
| _episode_predictions_frame | Return per-item prediction payload as a DataFrame. | x | x | x | x | x | x | x | 9 |
| _episodes_to_gluonts | Convenience wrapper that uses config defaults. | x | x | x | 7 | x | x | x | x |
| _evaluate_test_loader | Evaluate trainer on the test dataset when available. | x | 24 | x | x | x | x | x | x |
| _extract_bg_forecast | Extract BG (variate 0) from a model Forecast object. | x | x | x | x | x | x | 9 | x |
| _extract_episode_predictions | Helper for forecast generation/extraction. | x | x | x | x | x | 19 | x | x |
| _extract_ground_truth | Extract ground truth values from the end of the test data. | Potential pairwise combine candidate. | x | 10 | x | 11 | x | x | x |
| _extract_mean_forecasts | Helper for forecast generation/extraction. | x | x | x | 2 | x | x | x | x |
| _extract_patient_dataframes | Normalize training input into a patient->DataFrame mapping. | x | x | 34 | x | x | x | x | x |
| _extract_quantile_forecasts | Helper for quantile forecast extraction/validation. | Combine candidate with: _extract_mean_forecasts, _predict_impl, evaluate_probabilistic. | x | x | 8 | x | x | x | x |
| _extract_quantile_predictions | Return quantile predictions and fail if requested levels are unavailable. | Combine candidate with: _validate_registered_quantile_levels, _predict_with_context, _collect_batch_predictions. | x | x | x | x | x | x | 26 |
| _extract_timestamps | Get timestamps from DatetimeIndex or 'datetime' column. | x | x | x | x | x | x | 7 | x |
| _forecast_batch | Batch forecasts in a single model forward pass when possible. | x | x | x | x | 109 | x | x | x |
| _forecast_single | Single univariate forecast with optional wrapper-side normalization. | x | x | x | x | 60 | x | x | x |
| _get_callbacks | Get training callbacks. | x | 20 | x | x | x | x | x | x |
| _get_context_target_pairs | Build list of (context, target) from dataset name, DataFrame, or dict of DataFrames. | x | x | x | x | 171 | x | x | x |
| _get_covariate_columns | Return configured covariate columns that are available in df. | x | x | x | x | 5 | x | x | x |
| _get_input_columns | Input channel order for model: target first, then covariates. | x | x | x | x | 3 | x | x | x |
| _get_or_create_column_specifiers | Get cached column specifiers, creating them lazily on first use. | x | 5 | x | x | x | x | x | x |
| _get_or_create_predictor | Helper for forecast generation/extraction. | x | x | x | 7 | x | x | x | x |
| _get_target_column | Model-specific runtime helper. | x | x | x | x | 23 | x | x | x |
| _group_segments_by_patient | Group segmented arrays by original patient id. | x | x | 13 | x | x | x | x | x |
| _inverse_scale_predictions | Inverse scale predictions back to original units. | x | 102 | x | x | x | x | x | x |
| _iter_episode_dicts | Normalize dict/list training payloads to a flat episode list. | Combine candidate with: _prepare_training_tensors, _episodes_to_gluonts, build_gluonts_dataset. | x | x | 13 | x | x | x | x |
| _list_intermediate_checkpoints | Helper for checkpoint path/state handling. | x | x | x | x | x | 19 | x | x |
| _load_hf_model_weights | Model-specific runtime helper. | x | x | 16 | x | x | x | x | x |
| _load_preprocessor_checkpoint | Load preprocessor artifact from known checkpoint locations. | Combine candidate with: _save_preprocessor_checkpoint, _preprocessor_paths, _validate_preprocessor_schema. | 20 | x | x | x | x | x | x |
| _load_saved_checkpoint_config | Helper for checkpoint path/state handling. | x | x | 11 | x | x | x | x | x |
| _log_column_specifiers | Model-specific runtime helper. | x | 4 | x | x | x | x | x | x |
| _log_dataset_sizes | Helper for data normalization or dataset assembly. | x | 9 | x | x | x | x | x | x |
| _log_training_start | Helper for training setup/execution. | x | x | x | x | x | x | x | 8 |
| _longest_nan_run | Return the length of the longest contiguous True run in a boolean array. | x | x | 13 | x | x | x | x | x |
| _materialize_intermediate_checkpoints | Create standalone eval-ready snapshots from HF Trainer checkpoint-N dirs. | Combine candidate with: _list_intermediate_checkpoints, _write_snapshot_model_pt, _rel_symlink. | x | x | x | x | 58 | x | x |
| _normalize_context_array | Helper for context/covariate shaping. | x | x | x | x | 7 | x | x | x |
| _normalize_predict_input | Helper for forecast generation/extraction. | x | x | x | 6 | x | x | x | x |
| _normalize_split_config | Helper for runtime normalization/validation. | x | x | x | x | 10 | x | x | x |
| _normalize_training_input | Normalize supported training inputs to a DataFrame. | x | 19 | x | x | x | x | x | x |
| _optional_moment_import | Import MOMENTPipeline if momentfm is installed. | x | x | x | x | 11 | x | x | x |
| _predict_batch_fitted | Run one model batch prediction call and extract episode outputs. | x | x | x | x | x | 32 | x | x |
| _predict_batch_point | Generate point batch forecasts, chunked across episodes. | Combine candidate with: _predict_batch_quantiles, _run_forecast, _collect_batch_predictions. | x | x | x | x | x | 15 | x |
| _predict_batch_quantiles | Generate quantile batch forecasts, chunked across episodes. | Combine candidate with: _predict_batch_point, _run_forecast, _extract_quantile_predictions. | x | x | x | x | x | 27 | x |
| _predict_batch_zero_shot | Run zero-shot Chronos2Pipeline batch inference. | Combine candidate with: _prepare_zero_shot_context, _build_zero_shot_batch_context_tensor, _collect_zero_shot_batch_results. | x | x | x | x | 47 | x | x |
| _predict_impl | Run inference and return mean forecasts. | x | x | x | 53 | x | x | x | x |
| _predict_quantiles_impl | Internal quantile forecast logic shared by _predict and _predict_batch. | x | x | x | x | x | 16 | x | x |
| _predict_with_context | Run predictor inference on a prebuilt model context frame. | x | x | x | x | x | x | x | 11 |
| _prepare_autogluon_data | Format a raw inference DataFrame into a time-series frame. | x | x | x | x | x | 69 | x | x |
| _prepare_training_tensors | Convert training data to aligned tensor batches on CPU. | Combine candidate with: _iter_episode_dicts, _validate_covariate_columns, _convert_tensors_to_patched_dataset. | x | x | 111 | x | x | x | x |
| _prepare_zero_shot_context | Validate target column and return a model context tensor. | Combine candidate with: _build_zero_shot_pipeline, _zero_shot_forecast, _collect_zero_shot_batch_results. | x | x | x | x | 17 | x | x |
| _preprocessor_paths | Return supported preprocessor artifact locations for a checkpoint. | x | 6 | x | x | x | x | x | x |
| _rel_symlink | Model-specific runtime helper. | x | x | x | x | x | 4 | x | x |
| _require_initialized_model | Return model object or raise if weights are not initialized. | x | 5 | x | x | x | x | x | x |
| _resolve_eval_chunk_size | Resolve evaluation chunk size from config. | x | x | x | x | x | x | 11 | x |
| _resolve_freeze_backbone | Helper for runtime normalization/validation. | x | x | x | x | 7 | x | x | x |
| _resolve_predictor_path | Resolve predictor path from reference file with fallback semantics. | x | x | x | x | x | x | x | 20 |
| _resolve_target_columns | Resolve target columns and fail fast when no valid target remains. | x | 17 | x | x | x | x | x | x |
| _resolve_trainable_parameters | Helper for training setup/execution. | x | x | x | x | 13 | x | x | x |
| _resolve_training_input_dtype | Helper for training setup/execution. | x | x | 10 | x | x | x | x | x |
| _restore_trained_backbone | Restore best-validated backbone, falling back to final weights. | x | x | x | x | x | x | 29 | x |
| _run_forecast | Run forecaster and return BG predictions as numpy array. | x | x | x | x | x | x | 10 | x |
| _save_preprocessor_checkpoint | Persist preprocessor artifacts to checkpoint-compatible locations. | Combine candidate with: _load_preprocessor_checkpoint, _preprocessor_paths, _validate_preprocessor_schema. | 21 | x | x | x | x | x | x |
| _save_training_config | Helper for training setup/execution. | x | x | x | x | 4 | x | x | x |
| _segment_patient_data | Apply gap handling and segment data by patient. | x | x | 23 | x | x | x | x | x |
| _select_patch_size | Choose a fixed patch size for training. | x | x | x | 18 | x | x | x | x |
| _slice_masked_timeseries | Model-specific runtime helper. | x | x | x | x | x | x | 11 | x |
| _split_context_target_pairs | Helper for context/covariate shaping. | x | x | x | x | 30 | x | x | x |
| _split_train_val_patients | Split patient IDs into train/validation with at least one train patient. | x | x | 19 | x | x | x | x | x |
| _timestamps_to_seconds | Convert pandas timestamps to seconds-since-epoch float tensor. | x | x | x | x | x | x | 6 | x |
| _truncate_segment_for_training | Helper for training setup/execution. | x | x | 7 | x | x | x | x | x |
| _use_wrapper_normalization | Model-specific runtime helper. | x | x | x | x | 2 | x | x | x |
| _validate_covariate_columns | Helper for context/covariate shaping. | x | x | x | 12 | x | x | x | x |
| _validate_preprocessor_schema | Validate preprocessor schema required by the current runtime. | x | 14 | x | x | x | x | x | x |
| _validate_registered_quantile_levels | Helper for quantile forecast extraction/validation. | x | x | x | x | x | 15 | x | x |
| _warn_quantiles_not_supported | Helper for quantile forecast extraction/validation. | x | 12 | x | x | x | x | x | x |
| _write_checkpoint_config | Helper for checkpoint path/state handling. | x | x | 3 | x | x | x | x | x |
| _write_snapshot_model_pt | Model-specific runtime helper. | x | x | x | x | x | 11 | x | x |
| _zero_shot_forecast | Run zero-shot inference via Chronos2Pipeline. | x | x | x | x | x | 29 | x | x |
| build_gluonts_dataset | Build a GluonTS ``ListDataset`` from a list of episode dicts. | x | x | x | 63 | x | x | x | x |
| create_moirai_model | Factory function to create a ``MoiraiForecaster`` with sensible defaults. | x | x | x | 70 | x | x | x | x |
| create_moment_model | Factory function to create a Moment forecaster with sensible defaults. | x | x | x | x | 16 | x | x | x |
| create_timesfm_model | Factory function to create a TimesFM forecaster with sensible defaults. | x | x | 25 | x | x | x | x | x |
| evaluate | Evaluate model on test data using rolling-window evaluation. | x | x | 107 | x | x | x | x | x |
| evaluate_probabilistic | Evaluate model with full probabilistic outputs. | x | x | x | 111 | x | x | x | x |
| get_ttm_specific_info | Return model-specific runtime information for diagnostics. | x | 20 | x | x | x | x | x | x |
| predict_episodes | Evaluate model on a list of episodes and return per-episode metrics. | x | x | x | 51 | x | x | x | x |
| predict_single_window | Predict one step ahead for a single context window (for holdout/eval scripts). | x | x | x | x | 21 | x | x | x |
| predict_with_metadata | Make predictions and return dict with metadata. | x | x | x | x | 12 | x | x | x |
