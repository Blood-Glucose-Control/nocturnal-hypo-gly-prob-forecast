...
| _apply_univariate_patch | Monkey-patch pytorchts for univariate (target_dim=1) compatibility. | 1 |  | Retain family-specific (no safe consolidation yet) | _apply_univariate_patch | x | x | x | x | x | x | x | 29 | x | x | x | x | x | x | x |
...
| _build_batch_inputs | Build a padded MaskedTimeseries batch for episode-level inference. | 1 |  | Retain family-specific (no safe consolidation yet) | _build_batch_inputs | x | x | x | x | x | 53 | x | x | x | x | x | x | x | x | x |
| _build_context_matrix | Build finite context matrix [time, channels] from selected columns. | 7 |  | Retain family-specific (no safe consolidation yet) | _build_context_matrix | x | x | x | 9 | x | x | x | x | x | x | x | x | x | x | x |
...
| _build_finetuning_module | Construct the model Lightning finetuning module from current backbone. | 1 |  | Retain family-specific (no safe consolidation yet) | _build_finetuning_module | x | x | x | x | x | 18 | x | x | x | x | x | x | x | x | x |
...
| _build_train_val_arrays | Split patient segments into training and validation arrays. | 1 |  | Retain family-specific (no safe consolidation yet) | _build_train_val_arrays | x | 36 | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _build_variates | Extract BG target + covariate tensors from a single episode. | 3 |  | Retain family-specific (no safe consolidation yet) | _build_variates | x | x | x | x | x | 11 | x | x | x | x | x | x | x | x | x |
...
| _build_zero_shot_batch_context_tensor | Build a padded (N, 1, L) context tensor for zero-shot batch inference. | 1 |  | Retain family-specific (no safe consolidation yet) | _build_zero_shot_batch_context_tensor | x | x | x | x | 28 | x | x | x | x | x | x | x | x | x | x |
| _build_zero_shot_pipeline | Create the zero-shot inference pipeline with shared settings. | 1 |  | Retain family-specific (no safe consolidation yet) | _build_zero_shot_pipeline | 20 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| _build_zero_shot_pipeline_for_data | Build a zero-shot pipeline and return the resolved target column. | 2 |  | Retain family-specific (no safe consolidation yet) | _build_zero_shot_pipeline_for_data | 24 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| _capture_training_history | Capture in-memory trainer history for metadata/reporting. | 1 |  | Retain family-specific (no safe consolidation yet) | _capture_training_history | 16 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _collate_fn | Custom collator for HF Trainer. | 2 |  | Retain framework hook, extract internals where shared | _collate_fn | x | 14 | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _collect_zero_shot_batch_results | Collect per-episode numpy outputs from model zero-shot tensors. | 1 |  | Retain family-specific (no safe consolidation yet) | _collect_zero_shot_batch_results | x | x | x | x | 15 | x | x | x | x | x | x | x | x | x | x |
| _compute_input_size | Compute model's GRU input_size from the frequency string. | 1 |  | Retain family-specific (no safe consolidation yet) | _compute_input_size | x | x | x | x | x | x | x | 16 | x | x | x | x | x | x | x |
...
| _compute_validation_loss | Model-specific runtime helper. | 1 |  | Retain family-specific (no safe consolidation yet) | _compute_validation_loss | x | x | x | 26 | x | x | x | x | x | x | x | x | x | x | x |
| _configure_training_environment | Set runtime environment knobs for stable Trainer execution. | 1 |  | Retain family-specific (no safe consolidation yet) | _configure_training_environment | 18 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _create_column_specifiers | Create column specifiers for TimeSeriesPreprocessor. | 1 |  | Retain family-specific (no safe consolidation yet) | _create_column_specifiers | 42 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| _create_darts_model | Model-specific runtime helper. | 1 |  | Retain family-specific (no safe consolidation yet) | _create_darts_model | x | x | x | x | x | x | x | x | x | 35 | x | x | x | x | x |
...
| _create_training_artifacts | Create checkpoint and logger artifacts for fine-tuning. | 1 |  | Retain family-specific (no safe consolidation yet) | _create_training_artifacts | x | x | x | x | x | 20 | x | x | x | x | x | x | x | x | x |
...
| _empty_training_tensors | Helper for training setup/execution. | 1 |  | Retain family-specific (no safe consolidation yet) | _empty_training_tensors | x | x | 20 | x | x | x | x | x | x | x | x | x | x | x | x |
| _ensure_datetime_index | Ensure each patient frame has DatetimeIndex when available. | 1 |  | Retain family-specific (no safe consolidation yet) | _ensure_datetime_index | x | 15 | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _ensure_zs_pipeline | Lazily initialise the zero-shot Chronos2Pipeline. | 3 |  | Retain family-specific (no safe consolidation yet) | _ensure_zs_pipeline | x | x | x | x | 12 | x | x | x | x | x | x | x | x | x | x |
| _episode_ids_from | Return the array of episode IDs present in *data*. | 1 |  | Retain family-specific (no safe consolidation yet) | _episode_ids_from | x | x | x | x | 5 | x | x | x | x | x | x | x | x | x | x |
...
| _episodes_to_gluonts | Convenience wrapper that uses config defaults. | 1 |  | Retain family-specific (no safe consolidation yet) | _episodes_to_gluonts | x | x | 7 | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _extract_ground_truth | Extract ground truth values from the end of the test data. | 0 | YES | Extract shared inference helper | _shared_inference_extract_ground_truth | x | 10 | x | 11 | x | x | x | x | x | x | x | x | x | x | x |
...
| _get_context_target_pairs | Build list of (context, target) from dataset name, DataFrame, or dict of DataFrames. | 2 |  | Retain family-specific (no safe consolidation yet) | _get_context_target_pairs | x | x | x | 171 | x | x | x | x | x | x | x | x | x | x | x |
| _get_covariate_columns | Return configured covariate columns that are available in df. | 1 |  | Retain family-specific (no safe consolidation yet) | _get_covariate_columns | x | x | x | 5 | x | x | x | x | x | x | x | x | x | x | x |
| _get_input_columns | Input channel order for model: target first, then covariates. | 5 |  | Retain family-specific (no safe consolidation yet) | _get_input_columns | x | x | x | 3 | x | x | x | x | x | x | x | x | x | x | x |
| _get_or_create_column_specifiers | Get cached column specifiers, creating them lazily on first use. | 2 |  | Retain family-specific (no safe consolidation yet) | _get_or_create_column_specifiers | 5 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _get_target_column | Model-specific runtime helper. | 3 |  | Retain family-specific (no safe consolidation yet) | _get_target_column | x | x | x | 23 | x | x | x | x | x | x | x | x | x | x | x |
...
| _iter_episode_dicts | Normalize dict/list training payloads to a flat episode list. | 1 |  | Retain family-specific (no safe consolidation yet) | _iter_episode_dicts | x | x | 13 | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _load_darts_model | Model-specific runtime helper. | 1 |  | Retain family-specific (no safe consolidation yet) | _load_darts_model | x | x | x | x | x | x | x | x | x | 9 | x | x | x | x | x |
| _load_hf_model_weights | Model-specific runtime helper. | 1 |  | Retain family-specific (no safe consolidation yet) | _load_hf_model_weights | x | 16 | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _log_column_specifiers | Model-specific runtime helper. | 1 |  | Retain family-specific (no safe consolidation yet) | _log_column_specifiers | 4 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _log_training_start | Helper for training setup/execution. | 1 |  | Retain family-specific (no safe consolidation yet) | _log_training_start | x | x | x | x | x | x | 8 | x | x | x | x | x | x | x | x |
| _longest_nan_run | Return the length of the longest contiguous True run in a boolean array. | 1 |  | Retain family-specific (no safe consolidation yet) | _longest_nan_run | x | 13 | x | x | x | x | x | x | x | x | x | x | x | x | x |
...
| _optional_moment_import | Import MOMENTPipeline if momentfm is installed. | 2 |  | Retain family-specific (no safe consolidation yet) | _optional_moment_import | x | x | x | 11 | x | x | x | x | x | x | x | x | x | x | x |
...
| _prepare_zero_shot_context | Validate target column and return a model context tensor. | 1 |  | Retain family-specific (no safe consolidation yet) | _prepare_zero_shot_context | x | x | x | x | 17 | x | x | x | x | x | x | x | x | x | x |
...
| _rel_symlink | Model-specific runtime helper. | 5 |  | Retain family-specific (no safe consolidation yet) | _rel_symlink | x | x | x | x | 4 | x | x | x | x | x | x | x | x | x | x |
| _require_initialized_model | Return model object or raise if weights are not initialized. | 3 |  | Retain family-specific (no safe consolidation yet) | _require_initialized_model | 5 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| _resolve_eval_chunk_size | Resolve evaluation chunk size from config. | 4 |  | Retain family-specific (no safe consolidation yet) | _resolve_eval_chunk_size | x | x | x | x | x | 11 | x | x | x | x | x | x | x | x | x |
| _resolve_freeze_backbone | Helper for runtime normalization/validation. | 1 |  | Retain family-specific (no safe consolidation yet) | _resolve_freeze_backbone | x | x | x | 7 | x | x | x | x | x | x | x | x | x | x | x |
...
| _resolve_target_columns | Resolve target columns and fail fast when no valid target remains. | 1 |  | Retain family-specific (no safe consolidation yet) | _resolve_target_columns | 17 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| _resolve_trainable_parameters | Helper for training setup/execution. | 1 |  | Retain family-specific (no safe consolidation yet) | _resolve_trainable_parameters | x | x | x | 13 | x | x | x | x | x | x | x | x | x | x | x |
| _resolve_training_input_dtype | Helper for training setup/execution. | 1 |  | Retain family-specific (no safe consolidation yet) | _resolve_training_input_dtype | x | 10 | x | x | x | x | x | x | x | x | x | x | x | x | x |
| _restore_trained_backbone | Restore best-validated backbone, falling back to final weights. | 1 |  | Retain family-specific (no safe consolidation yet) | _restore_trained_backbone | x | x | x | x | x | 29 | x | x | x | x | x | x | x | x | x |
...
| _save_training_config | Helper for training setup/execution. | 1 |  | Retain family-specific (no safe consolidation yet) | _save_training_config | x | x | x | 4 | x | x | x | x | x | x | x | x | x | x | x |
...
| _select_patch_size | Choose a fixed patch size for training. | 2 |  | Retain family-specific (no safe consolidation yet) | _select_patch_size | x | x | 18 | x | x | x | x | x | x | x | x | x | x | x | x |
| _slice_masked_timeseries | Model-specific runtime helper. | 2 |  | Retain family-specific (no safe consolidation yet) | _slice_masked_timeseries | x | x | x | x | x | 11 | x | x | x | x | x | x | x | x | x |
...
| _timestamps_to_seconds | Convert pandas timestamps to seconds-since-epoch float tensor. | 3 |  | Retain family-specific (no safe consolidation yet) | _timestamps_to_seconds | x | x | x | x | x | 6 | x | x | x | x | x | x | x | x | x |
...
| _use_wrapper_normalization | Model-specific runtime helper. | 4 |  | Retain family-specific (no safe consolidation yet) | _use_wrapper_normalization | x | x | x | 2 | x | x | x | x | x | x | x | x | x | x | x |
| _validate_covariate_columns | Helper for context/covariate shaping. | 2 |  | Retain family-specific (no safe consolidation yet) | _validate_covariate_columns | x | x | 12 | x | x | x | x | x | x | x | x | x | x | x | x |
...
| create_moirai_model | Factory function to create a ``MoiraiForecaster`` with sensible defaults. | 0 | YES | No refs found: drop or move behind tests before keeping | create_moirai_model (move to <family>/utils.py) | x | x | 70 | x | x | x | x | x | x | x | x | x | x | x | x |
| create_moment_model | Factory to create a model with sensible defaults. | 0 | YES | No refs found: drop or move behind tests before keeping | create_moment_model (move to <family>/utils.py) | x | x | x | 16 | x | x | x | x | x | x | x | x | x | x | x |
| create_timesfm_model | Factory function to create a model with sensible defaults. | 0 | YES | No refs found: drop or move behind tests before keeping | create_timesfm_model (move to <family>/utils.py) | x | 25 | x | x | x | x | x | x | x | x | x | x | x | x | x |
| evaluate | Evaluate model on test data using rolling-window evaluation. | 1 |  | Move to family utility module | evaluate (move to <family>/utils.py) | x | 107 | x | x | x | x | x | x | x | x | x | x | x | x | x |
| evaluate_probabilistic | Evaluate model with full probabilistic outputs. | 0 | YES | No refs found: drop or move behind tests before keeping | evaluate_probabilistic (move to <family>/utils.py) | x | x | 111 | x | x | x | x | x | x | x | x | x | x | x | x |
| get_ttm_specific_info | Get model-specific model information. | 0 | YES | No refs found: drop candidate (manual review) | get_ttm_specific_info | 20 | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| predict_episodes | Evaluate model on a list of episodes and return per-episode metrics. | 0 | YES | No refs found: drop or move behind tests before keeping | predict_episodes (move to <family>/utils.py) | x | x | 51 | x | x | x | x | x | x | x | x | x | x | x | x |
| predict_single_window | Predict one step ahead for a single context window (for holdout/eval scripts). | 0 | YES | No refs found: drop or move behind tests before keeping | predict_single_window (move to <family>/utils.py) | x | x | x | 21 | x | x | x | x | x | x | x | x | x | x | x |
| predict_with_metadata | Make predictions and return dict with metadata. | 0 | YES | No refs found: drop or move behind tests before keeping | predict_with_metadata (move to <family>/utils.py) | x | x | x | 12 | x | x | x | x | x | x | x | x | x | x | x |
...
**Rows currently marked `No refs? = YES` (manual review required)**
...
**Decision logic**
- If a no-ref function is truly unused: remove with tests.
- If used externally/non-AST-detectable: move to explicit family utility module
  (`<family>/utils.py`) and add direct tests to preserve contract.
