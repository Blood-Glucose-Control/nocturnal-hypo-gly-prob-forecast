...
- `stats-rigor-module`: Promote `scripts/analysis/rebuttal_neurips2026/` A1–A9 into `src/evaluation/stats/` as first-class importable modules with unit tests + CI job that runs the fast ones on sample data on every PR. **Explicit migration guardrail:** any temporary package naming tied to "rebuttal" is transitional only and must be renamed to public-safe domain naming before upstream public release, with generic stats utilities split from project-specific loaders.
