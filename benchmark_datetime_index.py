...
from data.preprocessing.time_processing import get_train_validation_split, create_datetime_index
...
def create_large_test_data(num_patients=10, days_per_patient=90):
    ...
    for i in range(num_patients):
        ...
        np.random.seed(42 + i)
        n_points = len(times)
        glucose_values = 100 + 50 * np.sin(np.linspace(0, 60*np.pi, n_points)) + 20 * np.random.randn(n_points)
...
def benchmark_performance():
    ...
    print(f"Created dataset with {len(df)} records for {df['p_num'].nunique()} patients")
    ...
    try:
        ...
        print(f"   Records per second: {(total_train_records + total_val_records) / index_time:,.0f}")
    ...
    print(f"👥 Success rate: {info['patients_included']}/{info['total_patients']} patients")
