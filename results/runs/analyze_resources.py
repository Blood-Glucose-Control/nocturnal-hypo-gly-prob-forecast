...
class RunAnalyzer:
        ...
        try:
            ...
            for key, pattern in slurm_patterns.items():
                if match := re.search(pattern, content):
                    ...
                    if key in ['cpus_per_task', 'memory_per_node', 'memory_per_cpu', 'num_nodes', 'num_tasks', 'gpus_per_node']:
                        try:
                            value = int(value)
                        except ValueError:
                            pass
    ...
    def parse_training_log(self, run_dir: Path, run_data: Dict) -> Dict:
        ...
        try:
            ...
            if match := re.search(r"'train_samples_per_second': ([\d.]+)", content):
                run_data['performance_metrics']['train_samples_per_second'] = float(match.group(1))
            
            if match := re.search(r"'train_steps_per_second': ([\d.]+)", content):
                run_data['performance_metrics']['train_steps_per_second'] = float(match.group(1))
            
            if match := re.search(r"'train_loss': ([\d.]+)", content):
                run_data['performance_metrics']['final_train_loss'] = float(match.group(1))
            
            if match := re.search(r"'eval_loss': ([\d.]+)", content):
                run_data['performance_metrics']['final_eval_loss'] = float(match.group(1))
    ...
    def parse_gpu_utilization(self, run_dir: Path, run_data: Dict) -> Dict:
        ...
        try:
            ...
            if gpu_utils:
                run_data['performance_metrics']['avg_gpu_utilization'] = sum(gpu_utils) / len(gpu_utils)
            ...
            if memory_utils:
                run_data['performance_metrics']['avg_memory_utilization'] = sum(memory_utils) / len(memory_utils)
                run_data['performance_metrics']['max_memory_utilization'] = max(memory_utils)
            
            if temperatures:
                run_data['performance_metrics']['avg_temperature'] = sum(temperatures) / len(temperatures)
                run_data['performance_metrics']['max_temperature'] = max(temperatures)
            
            if power_draws:
                run_data['performance_metrics']['avg_power_draw'] = sum(power_draws) / len(power_draws)
    ...
    def print_summary(self):
        ...
        print(f"{'Job ID':<8} {'Node':<12} {'CPUs':<5} {'Mem(GB)':<8} {'Duration':<10} {'Steps/s':<8} {'GPU Util':<9} {'Status':<8}")
        print("-" * 80)
        ...
        for run in self.runs_data:
            ...
            steps_per_sec = run['performance_metrics'].get('train_steps_per_second', 'N/A')
            ...
            print(f"{job_id:<8} {node:<12} {cpus:<5} {mem_gb:<8} {duration:<10} {steps_per_sec:<8} {gpu_util_str:<9} {status:<8}")
    ...
    def print_resource_performance_analysis(self):
        ...
        for cpus, runs in sorted(cpu_groups.items()):
            ...
            avg_steps = [r['performance_metrics'].get('train_steps_per_second', 0) 
                        for r in runs if r['performance_metrics'].get('train_steps_per_second')]
            avg_gpu_util = [r['performance_metrics'].get('avg_gpu_utilization', 0) 
                           for r in runs if r['performance_metrics'].get('avg_gpu_utilization')]
            
            if avg_steps:
                avg_steps_val = sum(avg_steps) / len(avg_steps)
                avg_gpu_val = sum(avg_gpu_util) / len(avg_gpu_util) if avg_gpu_util else 0
                print(f"  {cpus} CPUs: {len(runs)} runs, avg {avg_steps_val:.2f} steps/s, avg {avg_gpu_val:.1f}% GPU util")
        ...
        for mem_gb, runs in sorted(mem_groups.items()):
            ...
            if avg_steps:
                avg_steps_val = sum(avg_steps) / len(avg_steps)
                print(f"  {mem_gb}GB+ Memory: {len(runs)} runs, avg {avg_steps_val:.2f} steps/s")
