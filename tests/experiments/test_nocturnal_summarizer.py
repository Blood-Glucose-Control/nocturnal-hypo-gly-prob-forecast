...
_RESULTS_TEMPLATE = {
    ...
    "total_episodes": 1830,
    "per_patient": [
        {"patient_id": "bro_75", "episodes": 6, "mse": 4.9, "rmse": 2.2, "mae": 1.6, "mape": 22.2},
        {"patient_id": "bro_93", "episodes": 5, "mse": 6.2, "rmse": 2.5, "mae": 2.1, "mape": 28.6},
    ],
}
...
class TestNocturnalParseRunDir:
    ...
    def test_mae_mape_mse_are_nan(self, tmp_path):
        ...
        run_dir = _make_run(tmp_path, "512ctx_72fh", "ttm", "2026-02-21_2239_d_m", _RESULTS_TEMPLATE)
        s = NocturnalSummarizer(tmp_path)
        row = s._parse_run_dir("512ctx_72fh", "ttm", run_dir)
        assert row is not None
    ...
    def test_returns_none_when_results_missing(self, tmp_path):
        run_dir = tmp_path / "nocturnal_forecasting" / "512ctx_72fh" / "ttm" / "2026-02-21_2000_d_m"
    ...
    def test_fallback_episode_count_from_per_patient(self, tmp_path):
        ...
        run_dir = _make_run(tmp_path, "512ctx_72fh", "ttm", "2026-02-21_2000_d_m", results)
        s = NocturnalSummarizer(tmp_path)
        row = s._parse_run_dir("512ctx_72fh", "ttm", run_dir)
        assert row is not None
    ...
    def test_reads_alternate_config_filename(self, tmp_path):
        ...
        run_dir = _make_run(
            tmp_path, "512ctx_72fh", "ttm", "2026-02-21_2000_d_m",
            _RESULTS_TEMPLATE, _CONFIG_TEMPLATE, config_filename="experiment_configs.json",
        )
        s = NocturnalSummarizer(tmp_path)
        row = s._parse_run_dir("512ctx_72fh", "ttm", run_dir)
        assert row is not None
...
class TestNocturnalSummarize:
    def test_produces_one_row_per_run(self, tmp_path):
        for i, dataset in enumerate(["brown_2019", "aleppo_2017"]):
            ...
            _make_run(tmp_path, "512ctx_72fh", "ttm", f"2026-02-21_200{i}_{dataset}_zeroshot", results)
    ...
    def test_summary_csv_columns(self, tmp_path):
        _make_run(tmp_path, "512ctx_72fh", "ttm", "2026-02-21_2239_d_m", _RESULTS_TEMPLATE)
    ...
    def test_skips_runs_without_results_file(self, tmp_path):
        _make_run(tmp_path, "512ctx_72fh", "ttm", "2026-02-21_2239_d_m", _RESULTS_TEMPLATE)
        incomplete = tmp_path / "nocturnal_forecasting" / "512ctx_72fh" / "ttm" / "2026-02-21_2300_d_m"
